...
const QuantumParticle = memo<QuantumParticleProps>(({ position, color, scale = 1 }) => {
...
const MacroscopicWave = memo(() => {
  ...
  return (
    <Torus ref={ref} args={[3, 0.1, 16, 64]} rotation={[Math.PI / 2, 0, 0]}>
      <meshStandardMaterial color="#C5A059" emissive="#C5A059" emissiveIntensity={0.5} transparent opacity={0.6} wireframe />
    </Torus>
  );
});

MacroscopicWave.displayName = 'MacroscopicWave';

const Fallback = () => (
  <div className="absolute inset-0 bg-gradient-to-b from-slate-900 to-slate-800" />
);

const HeroSceneContent = memo(() => (
  <>
...
HeroSceneContent.displayName = 'HeroSceneContent';
...
const QuantumComputerContent = memo(() => (
  <>
    ...
    <Float rotationIntensity={0.4} floatIntensity={0.2} speed={1}>
      <group rotation={[0, 0, 0]} position={[0, 0.5, 0]}>
        ...
        {/* Top Plate */}
        <Cylinder args={[1.2, 1.2, 0.1, 32]} position={[0, 1, 0]}>
          <meshStandardMaterial color="#C5A059" metalness={1} roughness={0.15} />
        </Cylinder>
        
        {/* Middle Stage */}
        <Cylinder args={[1, 1, 0.1, 32]} position={[0, 0.2, 0]}>
          <meshStandardMaterial color="#C5A059" metalness={1} roughness={0.15} />
        </Cylinder>
        
        {/* Bottom Stage (Mixing Chamber) */}
        <Cylinder args={[0.6, 0.6, 0.1, 32]} position={[0, -0.6, 0]}>
          <meshStandardMaterial color="#C5A059" metalness={1} roughness={0.15} />
        </Cylinder>

        {/* Connecting Rods */}
        <Cylinder args={[0.04, 0.04, 0.8, 8]} position={[0.5, 0.6, 0]}>
          <meshStandardMaterial color="#D1D5DB" metalness={0.8} roughness={0.2} />
        </Cylinder>
        <Cylinder args={[0.04, 0.04, 0.8, 8]} position={[-0.5, 0.6, 0]}>
          <meshStandardMaterial color="#D1D5DB" metalness={0.8} roughness={0.2} />
        </Cylinder>
        <Cylinder args={[0.04, 0.04, 0.8, 8]} position={[0, 0.6, 0.5]}>
          <meshStandardMaterial color="#D1D5DB" metalness={0.8} roughness={0.2} />
        </Cylinder>
        <Cylinder args={[0.04, 0.04, 0.8, 8]} position={[0, 0.6, -0.5]}>
          <meshStandardMaterial color="#D1D5DB" metalness={0.8} roughness={0.2} />
        ...
        {/* Lower Rods */}
        <Cylinder args={[0.03, 0.03, 0.8, 8]} position={[0.2, -0.2, 0]}>
          <meshStandardMaterial color="#D1D5DB" metalness={0.8} roughness={0.2} />
        </Cylinder>
        <Cylinder args={[0.03, 0.03, 0.8, 8]} position={[-0.2, -0.2, 0]}>
          <meshStandardMaterial color="#D1D5DB" metalness={0.8} roughness={0.2} />
        ...
        <Torus args={[0.7, 0.015, 12, 48]} position={[0, -0.2, 0]} rotation={[Math.PI/2, 0, 0]}>
          <meshStandardMaterial color="#B87333" metalness={0.8} roughness={0.3} />
        </Torus>
        <Torus args={[0.3, 0.015, 12, 48]} position={[0, -1, 0]} rotation={[Math.PI/2, 0, 0]}>
          <meshStandardMaterial color="#B87333" metalness={0.8} roughness={0.3} />
        </Torus>
...
QuantumComputerContent.displayName = 'QuantumComputerContent';

export const QuantumComputerScene: React.FC = memo(() => {
  return (
...
QuantumComputerScene.displayName = 'QuantumComputerScene';